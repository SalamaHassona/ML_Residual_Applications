...
def post_processing():
    ...
    with open(f'{args.model_labels}.pickle', 'rb') as OPTICS_labels_pickle:
        OPTICS_labels = pickle.load(OPTICS_labels_pickle)
    OPTICS_min = np.expand_dims(OPTICS_labels, axis=1)
    predict = np.concatenate((cr_point_list[:, 0:2], OPTICS_min), axis=1)
