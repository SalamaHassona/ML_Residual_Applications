...
def post_processing():
    ...
    for i in range(2, 24):
        with open(f'{args.model_labels}_v{i}.pickle', 'rb') as kmeans_labels_pickle:
            kmeans_labels = pickle.load(kmeans_labels_pickle)
        kmeans_min = np.expand_dims(kmeans_labels, axis=1)
        predict = np.concatenate((cr_point_list[:, 0:2], kmeans_min), axis=1)
