...
def generate_data_labels(cr_period_list, x0, y0, z0, c, dt, kd, kdmin):
    a = torch.from_numpy(cr_period_list[:, 0]).double().cpu()
    b = torch.from_numpy(cr_period_list[:, 1]).double().cpu()
...
if __name__ == "__main__":
    ...
    p.add_argument("--R1", type=float, default=6.00)
    p.add_argument("--R2", type=float, default=25.00)
    p.add_argument("--C1", type=float, default=2.80)
    p.add_argument("--C2", type=float, default=3.40)
