...
def generate_x_data(R, C, x0, y0, z0, data_range, L, dt, kd, kdmin):
    yp = torch.from_numpy(np.ones((data_range, kd-kdmin), dtype=np.double)).double().cpu()
    yp[:, 0] = y0
    ...
    for k in range(kdmin, kd - 1):
        ...
        yp[:, k - kdmin+1] = yy + ((ky1 + 2 * ky2 + 2 * ky3 + ky4) / 6) * dt
...
if __name__ == "__main__":
    ...
    p.add_argument("--out", type=str, default="y_data.pickle")
    args = p.parse_args()
    start = time.time()
    main()
    end = time.time()
